...
export class DeliveryAreaViewComponent implements OnInit {
	...
	constructor(private cityService: CityService, private snackBar: MatSnackBar,
		private deliveryAreaDataService: DeliveryAreaDataServiceService) {
		this.provinceList = [];
		this.districtList = [];
		this.cityList = [];
	...
	saveDeliveryInfo() {
		...
		if (this.emptyInitialSelectedList) {
			...
			this.cityService.addDeliveryCities(this.deliveryArea).subscribe(data => {
				this.deliveryAreaDataService.dataSetModified = true;
